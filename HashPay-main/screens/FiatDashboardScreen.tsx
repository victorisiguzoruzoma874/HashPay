...
type FiatTab = 'dashboard' | 'accounts' | 'savings' | 'transfers' | 'cards' | 'security' | 'bank-transactions';
...
const FiatDashboardScreen: React.FC<FiatDashboardScreenProps> = ({ onBack }) => {
    ...
    const [localBankTx, setLocalBankTx] = useState<any[]>([]);
    const [intBankTx, setIntBankTx] = useState<any[]>([]);
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        const fetchData = async () => {
            try {
                const [data, m, txs, g, c, ltx, itx] = await Promise.all([
                    ...
                    afriexClient.getBankTransactions('local'),
                    afriexClient.getBankTransactions('international')
                ]);
                setFiatData(data);
                setMetrics(m);
                setTransactions(txs);
    ...
    const menuItems = [
        ...
        { id: 'bank-transactions', label: 'Bank Txs', icon: 'account_balance' },
    ...
    const renderBankTransactions = () => (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
            <div className="flex justify-between items-center mb-12">
                <h2 className="text-3xl font-black font-display tracking-tight text-white">Bank Transactions Dashboard</h2>
                <div className="flex gap-4">
                    <button className="px-6 py-3 bg-primary rounded-xl text-xs font-black uppercase tracking-widest shadow-lg shadow-primary/20">Local Transfer</button>
                    <button className="px-6 py-3 bg-white/5 border border-white/10 rounded-xl text-xs font-black uppercase tracking-widest text-[#2176ff]">International</button>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                ...
                <div className="bg-surface-dark/20 rounded-[2.5rem] border border-white/5 p-8 glass">
                    <div className="flex items-center gap-4 mb-8">
                        ...
                        <div>
                            <h3 className="text-lg font-black text-white uppercase tracking-wider">Local Transactions</h3>
                            <p className="text-[10px] font-bold text-text-tertiary uppercase tracking-widest">Nigeria & Regional</p>
                        </div>
                    </div>

                    <div className="space-y-4">
                        {localBankTx.map((tx, i) => (
                            <div key={i} className="flex items-center justify-between p-5 bg-white/5 rounded-2xl border border-white/5 hover:bg-white/10 transition-all group">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <p className="text-xs font-black text-white">{tx.recipient}</p>
                                        <p className="text-[8px] font-bold text-text-tertiary uppercase tracking-widest">{tx.date} • {tx.reference}</p>
                                    </div>
                                </div>
                                <div className="text-right">
                                    <p className="text-sm font-black text-white">₦{tx.amount.toLocaleString()}</p>
                                    <span className={`text-[8px] font-black uppercase tracking-widest ${tx.status === 'Completed' ? 'text-green-400' : 'text-yellow-400'}`}>{tx.status}</span>
                                </div>
                            </div>
                        ))}
                    </div>
                ...
                <div className="bg-surface-dark/20 rounded-[2.5rem] border border-white/5 p-8 glass">
                    <div className="flex items-center gap-4 mb-8">
                        <div className="size-12 rounded-xl bg-[#2176ff]/20 flex items-center justify-center">
                            <span className="material-symbols-outlined text-[#2176ff] text-2xl">public</span>
                        </div>
                        <div>
                            <h3 className="text-lg font-black text-white uppercase tracking-wider">International</h3>
                            <p className="text-[10px] font-bold text-text-tertiary uppercase tracking-widest">Global Swift / Wire</p>
                        </div>
                    </div>
                    ...
                    <div className="space-y-4">
                        {intBankTx.map((tx, i) => (
                            <div key={i} className="flex items-center justify-between p-5 bg-white/5 rounded-2xl border border-white/5 hover:bg-white/10 transition-all group">
                                ...
                                <div className="text-right">
                                    <p className="text-sm font-black text-white">{tx.currency} {tx.amount.toLocaleString()}</p>
                                    <span className={`text-[8px] font-black uppercase tracking-widest ${tx.status === 'Completed' ? 'text-green-400' : tx.status === 'In Transit' ? 'text-blue-400' : 'text-yellow-400'}`}>{tx.status}</span>
                                </div>
                            </div>
                        ))}
                    </div>
            ...
            <div className="mt-12 grid grid-cols-3 gap-8">
                {[
                    { label: 'Total Local Outflow', value: '₦87,000', icon: 'south_east', color: 'text-error' },
                    { label: 'Total International', value: '$2,500', icon: 'public', color: 'text-primary' },
                    { label: 'Success Rate', value: '98.5%', icon: 'verified', color: 'text-success' },
                ].map((stat, i) => (
                    <div key={i} className="bg-surface-dark/30 rounded-3xl p-6 border border-white/5 glass flex items-center justify-between">
                        <div>
                            <p className="text-[9px] font-black text-text-tertiary uppercase tracking-[0.2em] mb-2">{stat.label}</p>
    ...
    const renderContent = () => {
        switch (activeTab) {
            ...
            case 'bank-transactions': return renderBankTransactions();
